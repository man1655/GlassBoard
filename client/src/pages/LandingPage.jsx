...
import { 
  ...
  useScroll, 
  useTransform, 
  // useMotionTemplate is not needed for this version
} from 'framer-motion';
import { 
  LayoutDashboard, Users, ShieldCheck, ArrowRight, 
  Activity, Bell, Settings, Database, Server, Code, Layers,
  Box 
} from 'lucide-react';
...
const LandingPage = () => {
  ...
  const techStack = [
    { name: "React.js", icon: <Code /> }, { name: "Node.js", icon: <Server /> },
    { name: "MongoDB", icon: <Database /> }, { name: "Express", icon: <Layers /> },
    { name: "Tailwind", icon: <LayoutDashboard /> }, { name: "Framer", icon: <Activity /> },
  ];
  ...
  return (
    <div className="relative min-h-screen text-white selection:bg-blue-500 selection:text-white">
      ...
      <div className="relative z-10">
        ...
        {/* NAVBAR */}
        <nav className="fixed top-0 w-full z-50 px-6 py-4 bg-black/10 backdrop-blur-md border-b border-white/10">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <h1 className="text-2xl font-bold tracking-tighter flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-blue-400 animate-pulse" />
              GlassBoard.
            </h1>
            <button onClick={() => navigate('/login')} className="px-6 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 rounded-full transition-all text-sm font-semibold">
              Login
            </button>
          </div>
        </nav>
        ...
        <section className="min-h-screen flex items-center justify-center px-6 pt-20">
          <div className="max-w-4xl mx-auto text-center relative z-10">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              <span className="px-4 py-1.5 rounded-full border border-blue-400/30 bg-blue-500/10 text-blue-300 text-sm font-medium mb-6 inline-block">
              ...
              <div className="flex flex-col md:flex-row gap-4 justify-center items-center">
                <button onClick={() => navigate('/register')} className="px-8 py-4 bg-white text-blue-900 font-bold rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center gap-2">
                  Get Started <ArrowRight className="w-5 h-5" />
                </button>
                <button className="px-8 py-4 bg-transparent border border-white/30 hover:bg-white/10 rounded-full transition-all text-white font-medium">
                  View Source Code
                </button>
              </div>
            </motion.div>
        ...
        {/* FOOTER */}
        <footer className="py-10 text-center text-white/30 border-t border-white/10 bg-black/20 backdrop-blur-md">
          <p className="mb-2">Designed & Developed for the MERN Stack Portfolio.</p>
          <p className="text-sm">© 2025 GlassBoard. Built with precision.</p>
        </footer>
